...
export default function SalesScreen() {
  ...
  const loadSalesOrders = async () => {
    try {
      const response = await api.get('/api/sales_orders?include={"customers":{"select":{"name":true}}}');
      setSalesOrders(response.data.data || []);
    } catch (error) {
      console.log('Failed to load sales orders:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <Card style={styles.orderCard}>
      <Card.Content>
        ...
        <Text variant="bodyMedium" style={styles.customer}>
          Customer: {item.customers?.name || 'N/A'}
        </Text>
    ...
    <View style={styles.container}>
      ...
      <FAB
        ...
        onPress={() => {
          ...
          console.log('Create new sales order');
        }}
      />
    </View>
  );
