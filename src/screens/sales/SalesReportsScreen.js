...
export default function SalesReportsScreen() {
  ...
  const fetchReports = async () => {
    try {
      ...
      // Fetch sales orders
      const ordersRes = await api.get('/api/sales_orders');
      const orders = Array.isArray(ordersRes.data) ? ordersRes.data : (ordersRes.data?.data || []);

      // Fetch sales deliveries  
      const deliveriesRes = await api.get('/api/sales_deliveries');
      const deliveries = Array.isArray(deliveriesRes.data) ? deliveriesRes.data : (deliveriesRes.data?.data || []);
      ...
      setReportData({
        overview: {
          ...
          totalReturns: 0, // TODO: Get from return_sales
          totalRevenue,
        },
        customers: [], // TODO: Group by customer
      ...
      >
          ...
          <View style={styles.statsGrid}>
            ...
            <View style={[styles.statCard, styles.statCardLarge]}>
              ...
              >
                ...
                <Text style={styles.statValue}>{formatCurrency(reportData.overview.totalRevenue)}</Text>
              </LinearGradient>
            </View>
            ...
            <View style={styles.statCard}>
              <View style={styles.statContent}>
                ...
                <Text style={styles.statNumber}>{reportData.overview.totalDeliveries}</Text>
                <Text style={styles.statTitle}>Phiếu Xuất</Text>
              </View>
            </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Khách Hàng Hàng Đầu</Text>
            <TouchableOpacity>
          ...
          {reportData.customers.length === 0 ? (
            ...
            reportData.customers.slice(0, 5).map((customer, index) => (
              <View key={index} style={styles.rankCard}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.rankName}>{customer.name}</Text>
                  <Text style={styles.rankSubtext}>{customer.count} đơn hàng</Text>
                </View>
                <Text style={styles.rankValue}>{formatCurrency(customer.value)}</Text>
              </View>
            ))
          )}
        </View>
