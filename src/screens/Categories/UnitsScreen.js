...
export default function UnitsScreen() {
  ...
  const fetchUnits = async () => {
    try {
      ...
      const unitsData = Array.isArray(response.data)
        ? response.data
        : (response.data?.data || []);
      setUnits(unitsData);
    } catch (error) {
  ...
  const handleDelete = (unitId) => {
    ...
    Alert.confirm(
      'Xác nhận xóa',
      `Bạn có chắc chắn muốn xóa đơn vị "${unit?.name}"?`,
      async () => {
        ...
        try {
          ...
          Alert.success('Xóa thành công!', `Đơn vị "${unitToDelete?.name}" đã được xóa.`);
        } catch (error) {
          if (unitToDelete) {
  ...
  const handleModalSubmit = async () => {
    ...
    try {
      ...
      if (modalMode === 'edit') {
        await api.put(`/api/units/${selectedUnit.id}`, dataToSend);
        setUnits(units.map(u => u.id === selectedUnit.id ? { ...u, ...dataToSend } : u));
        Alert.success('Cập nhật thành công!', 'Thông tin đơn vị đã được cập nhật.');
      } else {
        const response = await api.post('/api/units', dataToSend);
  ...
  const renderUnitItem = ({ item: unit }) => (
    <ListCard
      ...
      actions={[
        { label: 'Xem', icon: 'eye', color: '#1976d2', onPress: () => handleView(unit) },
        { label: 'Sửa', icon: 'pencil', color: '#4CAF50', onPress: () => handleEdit(unit) },
        { label: 'Xóa', icon: 'delete', color: '#F44336', onPress: () => handleDelete(unit.id) },
      ]}
    />
  );
