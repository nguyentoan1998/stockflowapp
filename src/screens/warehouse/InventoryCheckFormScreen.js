...
export default function InventoryCheckFormScreen() {
  const navigation = useNavigation();
  const route = useRoute();
  const { api } = useApi();
  const { user } = useAuth();
  const { mode = 'create', checkId } = route.params || {};
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (mode === 'edit' && checkId) {
        console.log('Loading inventory_transaction ID:', checkId);
        const checkRes = await api.get(`/api/inventory_transactions/${checkId}?include={"inventory_transaction_logs":true}`);
        console.log('Loaded data:', checkRes.data);
        const check = checkRes.data;
        setFormData({
          source_warehouse_id: check.source_warehouse_id,
          destination_warehouse_id: null,
          transaction_date: check.transaction_date?.split('T')[0],
          notes: check.description || '',
        });
        setItems((check.inventory_transaction_logs || []).map(log => ({
  ...
  const handleSaveForm = async () => {
    if (!formData.source_warehouse_id || items.length === 0) {
      Alert.error('Lỗi', 'Vui lòng chọn kho và thêm ít nhất 1 sản phẩm');
      return;
    }
    ...
    try {
      ...
      const code = `PKK-${year}`; // Server will append count (PKK = Phiếu Kiểm Kê)

      const dataToSend = {
        transaction_type: 'inventory_check',
      ...
      if (mode === 'edit') {
        await api.put(`/api/inventory_transactions/${checkId}`, dataToSend);
        Alert.success('Thành công!', 'Phiếu kiểm kê đã được cập nhật');
      } else {
        await api.post('/api/inventory_transactions', dataToSend);
        Alert.success('Thành công!', 'Phiếu kiểm kê đã được tạo');
      }
      ...
      <LinearGradient colors={['#1976d2', '#1565c0']} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerTitle}>
            {mode === 'create' ? 'Tạo phiếu kiểm kê' : 'Sửa phiếu kiểm kê'}
          </Text>
          <View style={{ width: 24 }} />
        </View>
      </LinearGradient>
      ...
      <Modal visible={sourceWarehouseModalVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Chọn kho kiểm kê</Text>
            <FlatList
