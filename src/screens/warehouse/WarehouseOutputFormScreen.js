...
export default function WarehouseOutputFormScreen() {
  const navigation = useNavigation();
  const route = useRoute();
  const { api } = useApi();
  const { user } = useAuth();
  const { mode = 'create', outputId } = route.params || {};
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (mode === 'edit' && outputId) {
        console.log('Loading inventory_transaction ID:', outputId);
        const outputRes = await api.get(`/api/inventory_transactions/${outputId}?include={"inventory_transaction_logs":true}`);
        console.log('Loaded data:', outputRes.data);
        const output = outputRes.data;
        setFormData({
          source_warehouse_id: output.source_warehouse_id,
          destination_warehouse_id: output.destination_warehouse_id,
          transaction_date: output.transaction_date?.split('T')[0],
          notes: output.description || '',
        });
        setItems((output.inventory_transaction_logs || []).map(log => ({
  ...
  const handleSaveForm = async () => {
    if (!formData.source_warehouse_id || items.length === 0) {
      Alert.error('Lỗi', 'Vui lòng chọn kho đi và thêm ít nhất 1 sản phẩm');
      return;
    }

    setSaving(true);
    try {
      // Generate code: PXH-YYYY-### (server will generate real count)
      const year = new Date().getFullYear();
      const code = `PXH-${year}`; // Server will append count
      ...
      if (mode === 'edit') {
        await api.put(`/api/inventory_transactions/${outputId}`, dataToSend);
      ...
      <LinearGradient colors={['#1976d2', '#1565c0']} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerTitle}>
            {mode === 'create' ? 'Tạo phiếu xuất' : 'Sửa phiếu xuất'}
          </Text>
          <View style={{ width: 24 }} />
        </View>
      </LinearGradient>
