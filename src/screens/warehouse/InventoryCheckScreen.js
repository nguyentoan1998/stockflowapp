...
export default function InventoryCheckScreen() {
  ...
  const fetchChecks = async () => {
    try {
      ...
      checksData = checksData.filter(item => item.transaction_type === 'inventory_check');
  ...
  const handleCreate = () => {
    navigation.navigate('InventoryCheckForm', { mode: 'create' });
  };

  const handleEdit = (check) => {
    navigation.navigate('InventoryCheckForm', { mode: 'edit', checkId: check.id });
  };

  const handleView = (check) => {
    navigation.navigate('InventoryCheckDetail', { checkId: check.id });
  };
