...
export default function WarehouseInputFormScreen() {
  const navigation = useNavigation();
  const route = useRoute();
  const { api } = useApi();
  const { user } = useAuth();
  const { mode = 'create', inputId } = route.params || {};
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (mode === 'edit' && inputId) {
        console.log('Loading inventory_transaction ID:', inputId);
        const inputRes = await api.get(`/api/inventory_transactions/${inputId}?include={"inventory_transaction_logs":true}`);
        console.log('Loaded data:', inputRes.data);
        const input = inputRes.data;
        setFormData({
          source_warehouse_id: input.source_warehouse_id,
          warehouse_id: input.destination_warehouse_id,
          transaction_date: input.transaction_date?.split('T')[0],
          notes: input.description || '',
        });
        setItems((input.inventory_transaction_logs || []).map(log => ({
  ...
  const handleSaveForm = async () => {
    if (!formData.warehouse_id || items.length === 0) {
      Alert.error('Lỗi', 'Vui lòng chọn kho đến và thêm ít nhất 1 sản phẩm');
      return;
    }

    setSaving(true);
    try {
      // Generate code: PNH-YYYY-### (server will generate real count)
      const year = new Date().getFullYear();
      const code = `PNH-${year}`; // Server will append count

      const dataToSend = {
        transaction_type: 'warehouse_input',
        source_warehouse_id: formData.source_warehouse_id ? parseInt(formData.source_warehouse_id) : null,
        destination_warehouse_id: parseInt(formData.warehouse_id),
      ...
      if (mode === 'edit') {
        await api.put(`/api/inventory_transactions/${inputId}`, dataToSend);
        Alert.success('Thành công!', 'Phiếu nhập đã được cập nhật');
      } else {
        await api.post('/api/inventory_transactions', dataToSend);
        Alert.success('Thành công!', 'Phiếu nhập đã được tạo');
      }
    ...
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#1976d2', '#1565c0']} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerTitle}>
            {mode === 'create' ? 'Tạo phiếu nhập' : 'Sửa phiếu nhập'}
          </Text>
          <View style={{ width: 24 }} />
        </View>
      </LinearGradient>
      ...
      <ScrollView style={styles.content}>
        <View style={styles.section}>
          <Text style={styles.label}>Kho đi (tùy chọn)</Text>
          <TouchableOpacity
        ...
        <View style={styles.section}>
          <Text style={styles.label}>Kho đến *</Text>
          <TouchableOpacity
            style={styles.selectBtn}
            onPress={() => setWarehouseModalVisible(true)}
          >
            <MaterialCommunityIcons name="warehouse" size={20} color="#666" />
            <Text style={styles.selectBtnText}>{getWarehouseName(formData.warehouse_id)}</Text>
            <Ionicons name="chevron-down" size={20} color="#999" />
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.itemsHeader}>
            <Text style={styles.label}>Sản phẩm nhập ({items.length})</Text>
      ...
      <Modal visible={sourceWarehouseModalVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <FlatList
              data={warehouses.filter(w => w.id !== formData.warehouse_id)}
              renderItem={({ item }) => (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.modalItem} onPress={() => { setFormData({ ...formData, warehouse_id: item.id }); setWarehouseModalVisible(false); }}>
                  <Text style={styles.modalItemText}>{item.name}</Text>
                </TouchableOpacity>
              )}
              keyExtractor={item => item.id?.toString()}
            />
            <TouchableOpacity style={styles.modalCloseBtn} onPress={() => setWarehouseModalVisible(false)}>
              <Text style={styles.modalCloseBtnText}>Đóng</Text>
            </TouchableOpacity>
          </View>
        </View>
