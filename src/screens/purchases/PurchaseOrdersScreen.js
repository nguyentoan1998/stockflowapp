...
export default function PurchaseOrdersScreen() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/purchase_orders?include={"suppliers":{"select":{"name":true,"code":true}}}&orderBy=[{"order_date":"desc"}]');
      ...
      Alert.error('Lỗi', 'Không thể tải danh sách đơn hàng mua');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      approved: { label: 'Đã xác nhận', color: '#3B82F6', bgColor: '#DBEAFE' },
    ...
    <View style={styles.card}>
      ...
      >
        ...
        <View style={styles.cardRow}>
          ...
          <Text style={styles.cardValue}>{order.suppliers?.name || 'N/A'}</Text>
        </View>
        ...
        {order.status === 'completed' && (
          <View style={styles.completedInfo}>
            ...
            <Text style={styles.completedInfoText}>
              Đơn hàng đã hoàn thành - Tự động cập nhật sau khi nhận hàng
            </Text>
          </View>
        )}
      </LinearGradient>
  ...
  const renderEmptyList = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>Chưa có đơn hàng mua</Text>
